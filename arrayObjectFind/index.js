...
function findEmail() {
  ...
  // 여기에 코드를 작성해 주세요.
  // 배열의 find Method는 조건에 해당되는 최초 record를 만나면 해당 object를 return 한다.
  // 배열의 filter method는 조건에 해당 되는 모든 record를 배열형태로 return 한다 .
  
  const user = data.find((obj) => obj.userName === nameValue && obj.phoneNumber === phoneValue);
  const message = user
